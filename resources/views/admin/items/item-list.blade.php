	...
	<div class="card-header">
		...
		<div class="row gap-md-0 gap-4">
			...
			<div class="col-md-6 text-md-end text-center">
				<a href="/admin/addnew/item/{{ $branchId }}" class="btn btn-primary">
					New
				</a>
			</div>
		</div>
		...
		<table id="searching_table" class="table table-hover">
			...
			<tbody class="table-border-bottom-0">
				...
				<tr>
					...
					<td>
						<img src="{{ '/images/branch-products/' . $branchId . '/250x250/' . $item->main_image }}" class="rounded" alt="Item" style="width:08rem;height:08rem;object-fit:cover;" />
					</td>
					<td>
						{{ $item->name }}
					</td>
					...
					<td>{{ $item->price }}</td>
					<td>
						<span class="badge bg-label-{{ ($item->instock == 1) ? 'success' : 'danger' }} me-1">
